...
import com.huiy.ssm.frame.persist.po.IPagePO;
...
public class Dialect {
	...
	private static Logger logger = LoggerFactory.getLogger(Dialect.class);
    ...
    public Object getOldParameterObject(){
    ...
    protected void init(){
        ...
        }else if (parameterObject instanceof IPagePO){
        	for (ParameterMapping parameterMapping : parameterMappings) {
        		String propValue = null;
				try {
					propValue = BeanUtils.getProperty(parameterObject, parameterMapping.getProperty());
				} catch (Exception  e) {
					logger.error("获取绑定变量失败,变量为："+parameterObject+"属性为:"+ parameterMapping.getProperty());
					throw new RuntimeException(e);
				}
                pageParameters.put(parameterMapping.getProperty(),propValue);
            }
        }else{
            for (ParameterMapping parameterMapping : parameterMappings) {
        ...
        logger.error("获取绑定变量,失败变量为："+pageParameters);
